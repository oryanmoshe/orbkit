...
- **Auto-Detection**: `detectBestRenderer()` returns CSS — the most feature-complete renderer (blur, wavy, interactive parallax, grain overlay). Canvas/WebGL are opt-in for performance-critical scenes with many orbs. Result is cached after first browser call. SSR always returns CSS (not cached). When `renderer="auto"`, OrbScene defers detection to `useEffect` to avoid SSR/CSR hydration mismatches — initial render always uses CSS.
