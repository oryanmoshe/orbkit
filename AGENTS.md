...
- **Auto-Detection**: `detectBestRenderer()` probes WebGL2 > WebGL1 > Canvas 2D > CSS. Result is cached after first browser call. SSR always returns CSS (not cached). When `renderer="auto"`, OrbScene defers detection to `useEffect` to avoid SSR/CSR hydration mismatches — initial render always uses CSS.
