 ...
 * This file re-exports them as TypeScript string constants for bundling.
 *
 * When editing shaders, modify the .glsl files and copy changes here.
 */

// ─── Vertex Shaders ─────────────────────────────────────────────────────────

/** Fullscreen triangle vertex shader for WebGL2 (uses gl_VertexID, no buffers needed) */
export const VERTEX_SHADER_WEBGL2 = /* glsl */ `#version 300 es
void main() {
  vec2 pos = vec2(
...
/**
 * Fragment shader body (without preamble).
 * Preamble is prepended per WebGL version — see FRAGMENT_SHADER_WEBGL2/WEBGL1.
 */
const FRAGMENT_SHADER_BODY = /* glsl */ `
